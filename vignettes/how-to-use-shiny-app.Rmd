...
```{r, include = FALSE}
screenshot <- function(x) system.file("screenshots", x, package = "opencodecounts")
...
```{r, echo=FALSE, out.width="100%", fig.cap="**Figure 1.**  Select dataset for anlysis."}
knitr::include_graphics(screenshot("shiny_app_datasets.png"))
```
...
```{r, echo=FALSE, out.width="100%", fig.cap="**Figure 2.** Search codes by specific code, description, or load codelist."}
knitr::include_graphics(screenshot("shiny_app_select_codes.png"))
```
...
```{r, echo=FALSE, out.width="100%", fig.cap="**Figure 4.** Main page with tabs for *Trends over time*, *Usage table*, and *Selected codes*."}
knitr::include_graphics(screenshot("shiny_app_views.png"))
```
